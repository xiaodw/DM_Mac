...
+ (void)loginSystem:(NSString *)account psd:(NSString *)password block:(void(^)(BOOL result))complectionBlock {
    ...
    // http://test.api.cn.discovermelody-app.com/user/login // DM_User_Loing_Url
    [[DMHttpClient sharedInstance] initWithUrl:@"http://test.api.cn.discovermelody-app.com/user/login" parameters:dic method:DMHttpRequestPost dataModelClass:[DMLoginDataModel class] isMustToken:NO success:^(id responseObject) {
...
+ (void)getHomeCourseData:(NSString *)type block:(void(^)(BOOL result, NSArray *array))complectionBlock {
    ...
    NSString *url = @"http://test.api.cn.discovermelody-app.com/lesson/scourseIndex";//DM_User_Scourse_Index_Url;
    if (type.intValue == 1) {
...
+ (void)commitQuestAnswer:(NSString *)lessonId answers:(NSArray *)answerArray block:(void(^)(BOOL result))complectionBlock
{
    ...
    [DMHttpClient sharedInstance].blockSuccessMsg = ^(NSString *msg) {
     //   [DMTools showSVProgressHudCustom:@"hud_success_icon" title:msg];
    };
}
